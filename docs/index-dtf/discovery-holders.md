...
- `/discover/dtfs`: canonical aggregate discovery used by Register list/status screens. Pass `chainId` to scope it to one chain.

The chain-scoped `/discover/dtf` endpoint is deprecated in reserve-api (popularity-filtered browse view that can omit supported DTFs) and the SDK no longer wraps it.
