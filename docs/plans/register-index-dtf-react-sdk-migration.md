...
Status: pending. This is an implementation plan for moving Register's Index DTF flows onto `@reserve-protocol/sdk` and `@reserve-protocol/react-sdk`. It is not proof that every referenced hook or SDK method exists.
...
Register already uses the local React SDK package:

- `register/package.json` links `@reserve-protocol/react-sdk` to `../dtf-sdk/packages/react-sdk`.
...
- `sdk.index.get`, `getDtf`, `getFull`, `getBasket`, `getBasketSnapshot`, `getVersion`, `getTotalSupply`, `getTotalAssets`, `getBrand`, `getMandate`, `getPrice`, `getPrices`, `getPriceHistory`.
...
## Query Key Cleanup For Hook Additions

React SDK currently has two query-key styles:

- `packages/react-sdk/src/query-keys.ts` exports `dtfQueryKeys` and is used by `query-options.ts` and governance hooks.
- `packages/react-sdk/src/index-dtf-query-options.ts` has a local `key(scope, params)` helper and is used by the newer extra hooks.

Do this before adding more extra hooks. Do not block a Register migration step on this cleanup unless that step adds or changes React SDK query options.

- Move status, exposure, transactions, revenue, issuance-state, rebalances, current-rebalance, and vote-lock-state keys into `dtfQueryKeys.index`.
- Add `dtfQueryKeys.index.discover` for Index discovery with `performance` and `brand` params. Do not reuse generic `dtfQueryKeys.discover` for this hook.
- Add `dtfQueryKeys.portfolio.get`, `dtfQueryKeys.portfolio.history`, and `dtfQueryKeys.portfolio.transactions` before adding portfolio hooks.
- Move the existing `accountPortfolioQueryOptions` / `useAccountPortfolio` off the local portfolio key when adding `dtfQueryKeys.portfolio.get`.
- Keep keys normalized with `normalizeQueryKeyValue` through the existing key helper path.
- Decide whether `indexDtfQueryOptions` and `fullIndexDtfQueryOptions` are intentionally aliases. If not, move `indexDtfQueryOptions` to `dtfQueryKeys.index.dtf(params)` to avoid a cache collision.
- Expect disabled extra-hook query keys to change from an `undefined` segment to the existing `"default"` key-param convention.
- Keep public consumers importing from the package root. Optionally re-export extra query options through `query-options.ts` during cleanup.
- Add tests for moved query keys, query options, and root exports.

This avoids baking a second cache-key convention into Register while the migration is expanding.
...
| Platform fee | wagmi `daoFeeRegistry()` and `getFeeDetails()` | add/use `useIndexDtfPlatformFee` or select from `useIndexDtfRevenue` if it includes the needed exact value | Preserve Register fallback fee behavior only if SDK read fails and product wants fallback display. |
...
| Deprecation status | `useDTFStatus`, direct `/discover/dtfs`, `KNOWN_DEPRECATED` snapshot | `useIndexDtfStatus` for Index route status; add `useIndexDtfStatuses` only if batch status is needed | Limit SDK replacement to Index DTF consumers. Keep or split the generic Register hook for Yield/Earn flows. Keep `KNOWN_DEPRECATED` if instant pre-load behavior matters. |
...
| Governance proposal list/detail | partially React SDK | Continue migration to `useIndexDtfProposalList`, `useIndexDtfProposal`, voting state hooks | Register display-state derivation remains product-owned unless React SDK deliberately exposes a display helper. |
...
| Liquidity checks | product hooks and Zapper/token APIs | Product-owned unless SDK Zapper/token API is implemented | These are UI risk checks, not core SDK contract truth. |
| Settings basics | wagmi `bidsEnabled` etc. | `useIndexDtfRevenue`, new hooks for `getBidsEnabled`, `getRebalanceControl`, `getApprovedRevenueTokens` if needed | Add focused hooks instead of generic read-contract wrappers. |
...
## React SDK Hooks To Add

Add these only where core SDK methods already exist:

- `indexDtfPlatformFeeQueryOptions` / `useIndexDtfPlatformFee` for `sdk.index.getPlatformFee`.
- `indexDtfRebalanceControlQueryOptions` / `useIndexDtfRebalanceControl` for `sdk.index.getRebalanceControl`.
- `indexDtfBidsEnabledQueryOptions` / `useIndexDtfBidsEnabled` for `sdk.index.getBidsEnabled`.
- `indexDtfPendingFeeSharesQueryOptions` / `useIndexDtfPendingFeeShares` for `sdk.index.getPendingFeeShares`.
- `indexDtfApprovedRevenueTokensQueryOptions` / `useIndexDtfApprovedRevenueTokens` for `sdk.index.getApprovedRevenueTokens`.
- `indexDtfHoldersQueryOptions` / `useIndexDtfHolders` if holder counts are still read directly in Register.
- `indexDtfStatusesQueryOptions` / `useIndexDtfStatuses` only if Register needs batch Index DTF statuses.
...
- `indexDtfTotalSupplyQueryOptions` and `indexDtfTotalAssetsQueryOptions` only if Register needs standalone live values outside `useCurrentIndexDtf` or issuance state.
- `portfolioHistoryQueryOptions` / `useAccountPortfolioHistory` for `sdk.portfolio.getHistory` after SDK period typing and response shape match Register.
- `portfolioTransactionsQueryOptions` / `useAccountPortfolioTransactions` for `sdk.portfolio.getTransactions`.
...
### 1. React SDK Query-Key Consolidation

Work in `dtf-sdk`:

- Move the extra hook keys into `dtfQueryKeys.index` and add `dtfQueryKeys.portfolio`.
- Update `index-dtf-query-options.ts` to use `createDtfQueryOptions` and `dtfQueryKeys`.
- Fix or document the `indexDtfQueryOptions` / `fullIndexDtfQueryOptions` cache-key alias.
- Add missing tests for status, exposure, transactions, revenue, issuance state, rebalances, current rebalance, vote-lock state, portfolio keys/options, and root exports.
- Run React SDK tests for query keys/options/exports.

### 2. Low-Risk Read Hooks

Work in `dtf-sdk`, then Register:

- Add React SDK hooks for platform fee, rebalance control, bids enabled, pending fee shares, approved revenue tokens, and portfolio transactions.
...
- `PlatformFeeUpdater` -> `useIndexDtfPlatformFee`.
...
- Replace Index DTF route/list status with `useIndexDtfStatus` or a new `useIndexDtfStatuses` hook.
...
- Keep Register's proposal display-state derivation from `register/src/lib/governance.ts` unless a React SDK display helper is intentionally added.
- Replace proposal list/detail reads with existing React SDK hooks where not already migrated.
...
- Manual smoke test the migrated flow on the intended chain.
- For transaction builders, inspect prepared `to`, `data`, `value`, chain ID, args, and approval spender before sending on a wallet.
