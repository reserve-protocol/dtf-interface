...
- `governance-overview-hooks.ts` wraps the cross-DTF governance reads (Index and Yield) plus `useIndexDtfVoteLockDaos`. Their params are optional, so the queries are always enabled; the proposal feeds and `useIndexDtfVoteLockLifetimeTotals` sit at the static tier (heavy, indexed data).
