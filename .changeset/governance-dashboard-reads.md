...
- `sdk.index.getProposalFeed`, `getTopVoters`, `getGovernanceActivity`: proposals, most active voters, and a merged event feed (votes, proposal lifecycle, vault deposits/withdrawals) across every Index DTF chain, with derived proposal state and the DTFs each governance controls. Attribution resolves from the DTF side (owner/trading governances, current or legacy, then the vault), so proposals on a governance or vault a DTF has since replaced still name that DTF.
