...
- Governance→DTF attribution (`governance/governed-dtfs.ts`) is resolved from the DTF side, one directory walk per chain: a DTF's owner or trading governance, current or legacy (`legacyAdmins`, `legacyAuctionApprovers`), controls that one DTF; a vault DAO governance (no such match) controls every DTF currently on its vault. Resolving from the vault side broke after vault migrations (old MAG7 vault on Base lists no DTFs). Vaults without an indexed `underlying` are skipped, never invented.
