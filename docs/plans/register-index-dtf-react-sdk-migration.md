...
Shipped: platform fee, rebalance control, bids enabled, pending fee shares, approved revenue tokens, holders, total supply/assets, portfolio history/transactions, rebalance detail, active/latest auction, bid quote, and rebalance-liquidity query options/hooks. The full/current DTF result now includes platform fee and status.
...
- Put route status on `useCurrentIndexDtf`; use discovery/batch status for lists. Do not issue one paginated `useIndexDtfStatus` query per row or route.
