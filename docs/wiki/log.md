...
- Post-PR review by Luis found three P2s, all reproduced live: proposals on a migrated-away vault lost DTF attribution (fixed with a DTF-side directory per chain), stRSR transfers showed as stake/unstake pairs (fixed by reading STAKE/UNSTAKE `Entry` rows instead of balance records), and withdrawals to another receiver appeared twice (fixed by dropping zero-share bookkeeping records). Lesson: derived-from lists on the current entity cannot answer historical questions; resolve from the side that keeps history.
- PR #44 review (JuampiRombola) plus Luis's "are we fetching all proposals?": feeds now take a creation-time window (`since`/`until`, default last 60 days), sit at the static cache tier in react-sdk, dedupe by `id` after a skip walk, and the DTF directory is memoized per client and chain for 60s so four reads mounting together share one walk per chain. The dashboard reads 60 days for the open tables and 12 months for the historical section; its all-time totals became twelve-month totals, since all-time counts belong to the Governance counters, not to a proposal walk.
