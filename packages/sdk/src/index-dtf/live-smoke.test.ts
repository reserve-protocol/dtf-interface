...
smokeDescribe("Index DTF live smoke", () => {
  ...
  for (const smokeCase of CASES) {
    it(`${smokeCase.label}: reads core DTF, market, basket, status, and history surfaces`, async () => {
    ...
    it(`${smokeCase.label}: reads governance, proposals, decoded calldatas, voters, and delegates`, async () => {
      ...
      const [proposals, allProposals, delegates, guardians, legacyVoteLocks, voterState, proposerState] =
        await Promise.all([
          ...
          sdk.index.getAllProposals({ chainId: BASE_CHAIN_ID, limit: 5 }),
      ...
      expect(
        proposalVotes.forVotes.raw + proposalVotes.againstVotes.raw + proposalVotes.abstainVotes.raw,
      ).toBeGreaterThanOrEqual(0n);
      expect(proposalVoterState.account).toBe(SMOKE_ACCOUNT);

      const vote = sdk.index.prepareVote({
        chainId: BASE_CHAIN_ID,
        governance: detail.governance,
      ...
      expect(decodeFunctionData({ abi: dtfIndexGovernanceAbi, data: voteWithReason.data }).functionName).toBe(
        "castVoteWithReason",
      );
      expect(decodeFunctionData({ abi: dtfIndexGovernanceAbi, data: voteWithParams.data }).functionName).toBe(
    ...
    it(`${smokeCase.label}: validates optimistic governance expectations`, async () => {
      ...
      const optimistic = await sdk.index.getOptimisticGovernance({
        chainId: BASE_CHAIN_ID,
        governance: governance.address,
      });
      ...
      const [roles, throttleCharges, targets] = await Promise.all([
        ...
        sdk.index.getProposalThrottleCharges({
          chainId: BASE_CHAIN_ID,
          governance: governance.address,
          account: SMOKE_ACCOUNT,
        }),
      ...
      expect(
        decodeFunctionData({ abi: dtfIndexGovernanceOptimisticAbi, data: submitOptimistic.data }).functionName,
    ...
    it(`${smokeCase.label}: validates revenue, issuance, vote-lock, and rebalance surfaces`, async () => {
      ...
      const lockDeposit = sdk.index.prepareVoteLockDeposit({
        chainId: BASE_CHAIN_ID,
        stToken,
        amount: 1n,
        delegateToSelf: true,
      });
      const withdraw = sdk.index.prepareVoteLockWithdraw({
        chainId: BASE_CHAIN_ID,
        stToken,
        amount: 1n,
        account: SMOKE_ACCOUNT,
      ...
      expect(decodeFunctionData({ abi: dtfIndexStakingVaultAbi, data: lockDeposit.data }).functionName).toBe(
        "depositAndDelegate",
      );
    ...
    it(`${smokeCase.label}: builds settings, basket, DAO, timelock, and deploy proposal payloads`, async () => {
      ...
      const [settings, basketSettings, daoSettings, basketProposal] = await Promise.all([
        ...
        sdk.index.buildBasketProposal({
          ...
          prices: Object.fromEntries(
            Object.values(full.basket).map((asset) => [asset.token.address.toLowerCase(), asset.price]),
          ),
          tokenDecimals: Object.fromEntries(
            Object.values(full.basket).map((asset) => [asset.token.address.toLowerCase(), asset.token.decimals]),
          ),
          priceErrors: Object.fromEntries(
            Object.values(full.basket).map((asset) => [asset.token.address.toLowerCase(), 0.5]),
          ),
      ...
      expect(decodeFunctionData({ abi: dtfIndexAbi, data: basketProposal.calldatas[0]! }).functionName).toBe(
        "startRebalance",
      );
      ...
      expect(decodeFunctionData({ abi: dtfIndexGovernanceAbi, data: updateTimelock.data }).functionName).toBe(
        "updateTimelock",
      );
      ...
      const deployBasicDetails = {
        ...
        assets: Object.values(full.basket)
          .slice(0, 2)
          .map((asset) => asset.token.address),
        amounts: Object.values(full.basket)
          .slice(0, 2)
          .map(() => 1n),
      ...
      expect(decodeFunctionData({ abi: indexDtfDeployerAbi, data: governedDeploy.data }).functionName).toBe(
        "deployGovernedFolio",
      );
...
function getGovernanceAuthority(
...
function getCaseGovernance(dtf: IndexDtf, expectsOptimisticGovernance: boolean): GovernanceAuthority {
  const governance =
    getGovernanceAuthority(dtf.governance.rebalance.primary) ?? expectGovernanceAuthority(dtf.governance.admin.primary);
